...
var Dismantler = {
    ...
    has_structures_to_dismantle(room, pos) {
        var flags = room.lookForAt(LOOK_FLAGS, pos);
     ...
     */
    find_sites(room) {
        var flags = room.find(FIND_FLAGS, { filter: Dismantler.is_dismantler_flag });
        console.log("Found " + flags.length + " dismantler flags.");
        var structures = [];
        for (var i in flags) {
            var flag = flags[i];
            var flaggedStructures = room.lookForAt(LOOK_STRUCTURES, flag.pos);
    ...
    work: function(worker, site = null) {
        ...
        if (!site && !worker.memory.site) {
            var sites = Dismantler.find_sites(worker.room);
            if (sites.length == 0) {
